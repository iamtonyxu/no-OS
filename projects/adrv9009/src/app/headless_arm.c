...
uint32_t zero_lut_iq[DAC_BUFFER_SAMPLES];
...
#endif
	/* receive command and take actions */
...
#else
					wr_data[6] = 0x12; wr_data[7] = 0x34; wr_data[8] = 0x56; wr_data[9] = 0x78;
#endif
					no_os_uart_write(uart_desc, wr_data, bytes_number);
					break;
				case 0x5C: // download waveform
					...
					no_os_mdelay(10);
					break;
				case 0x5D: // read capture
					bytes_number = (wr_data[1] << 2*8) | (wr_data[2] << 1*8) | (wr_data[3] << 0*8);
					uint16_t samples_number = bytes_number / 4;
					if(samples_number > ADC_BUFFER_SAMPLES)
					{
						samples_number = ADC_BUFFER_SAMPLES;
					}
#ifndef ADRV9008_2
					...
					no_os_mdelay(10);
					break;
				case 0x60: // get EnabledTrackingCals
					...
					no_os_uart_write(uart_desc, wr_data, bytes_number);
					break;
				case 0x61: // set EnabledTrackingCals
					for(int i = 1; i < 5; i++) {
						enableMask |= (wr_data[i + 1] << (8 * (4 - i)));
					}
					TALISE_setEnabledTrackingCals(&tal[TALISE_A], enableMask);
					break;
				default:
					/* do nothing */
