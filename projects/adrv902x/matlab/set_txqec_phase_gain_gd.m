...
gain2Bytes = typecast(swapbytes(uint16(gain(2))), 'uint8');% big-endian
gain3Bytes = typecast(swapbytes(uint16(gain(3))), 'uint8');% big-endian
gain4Bytes = typecast(swapbytes(uint16(gain(4))), 'uint8');% big-endian
gain5Bytes = typecast(swapbytes(uint16(gain(5))), 'uint8');% big-endian

phaseBytes = typecast(swapbytes(uint16(phase)), 'uint8');% big-endian
gd1Bytes = typecast(swapbytes(uint16(gd(1))), 'uint8');% big-endian
gd2Bytes = typecast(swapbytes(uint16(gd(2))), 'uint8');% big-endian
