...
/* Base address table for TX SPI Registers */
static const uint32_t TxSpiBaseAddr[4] =
{
    ADRV9025_ADDR_TX0_BASE_ADDR,       /* Tx0 SPI Base address */
    ADRV9025_ADDR_TX1_BASE_ADDR,       /* Tx1 SPI Base address */
    ADRV9025_ADDR_TX2_BASE_ADDR,       /* Tx2 SPI Base address */
    ADRV9025_ADDR_TX3_BASE_ADDR,       /* Tx3 SPI Base address */
};
...
void ADRV9025_set_phase_gain_gd(void *devHalInfo, uint8_t chan, txqec_params_t param, int16_t value) 
{
    ...
    switch (param) {
        case PARAM_PHASE: /* Adjust phase of Q */
            ...
            __NOP(); __NOP(); __NOP();
            adi_adrv9025_SpiByteRead(devHalInfo, base + IDX_LO_DELAY_CORR_DAC_0, &tempdata);
            tempdata = (tempdata & ~BITM_SPI_TX1_LO_DELAY_CORR_DAC_0_TX1_LO_DELAY_LATCH_EN) | 0x0u;
            adi_adrv9025_SpiByteWrite(devHalInfo, base + IDX_LO_DELAY_CORR_DAC_0, tempdata);
            break;
...
void ADRV9025_get_phase_gain_gd(void *devHalInfo, uint8_t chan, txqec_outputs_t *out, uint32_t param_mask) 
{
    ...
    if (param_mask & MASK_PARAM_PHASE) {
        ...
        adi_adrv9025_SpiByteRead(devHalInfo, base + IDX_LO_DELAY_CORR_DAC_1, &tempdata);
    ...
    if (param_mask & (MASK_PARAM_GROUP_DELAY | MASK_PARAM_GROUP_DELAY_2ND)) {
        adi_adrv9025_SpiByteRead(devHalInfo, base + IDX_ABBF_C1_QEC_TRIM_MSBS, &tempdata);
        out->gd[0]  = (uint16_t)(tempdata & 0x3) << 8;
        adi_adrv9025_SpiByteRead(devHalInfo, base + IDX_ABBF_C1_QEC_TRIM_LSBS, &tempdata);
        out->gd[0] |= (uint16_t)(tempdata & 0xFF);

        adi_adrv9025_SpiByteRead(devHalInfo, base + IDX_ABBF_C2_QEC_TRIM_MSBS, &tempdata);
        out->gd[1]  = (uint16_t)(tempdata & 0x3) << 8;
        adi_adrv9025_SpiByteRead(devHalInfo, base + IDX_ABBF_C2_QEC_TRIM_LSBS, &tempdata);
