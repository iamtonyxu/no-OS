/**
 * \file fr9009_error.c
 * \brief Contains FR9009 API error information functions
...
#else
	uint32_t i = 0;
	brErrorFunction_t *talFunctionTable =
...
static const char* talCallbackErrorMessage(fr9009Device_t *device,
...
uint32_t TALISE_getErrCode(fr9009Device_t *device, uint32_t* errSrc,
			   uint32_t* errCode)
{
...
const char* TALISE_getErrorMessage(uint32_t errSrc, uint32_t errCode)
{
	...
	if(errSrc == FR_ERRSRC_FRHAL) {
		switch (errCode) {
		case FRHAL_OK:
			return "";
		case FRHAL_SPI_FAIL:
			return "FRHAL: SPI Failure\n";
		case FRHAL_GPIO_FAIL:
			return "FRHAL: GPIO Failure\n";
		case FRHAL_TIMER_FAIL:
			return "FRHAL:Timing Function Failure\n";
		case FRHAL_WAIT_TIMEOUT:
			return "FRHAL:HAL Access Timed out\n";
		case FRHAL_GEN_SW:
			return "FRHAL:HAL Function Soft Error\n";
		case FRHAL_WARNING:
			return "FRHAL:HAL Function Soft Warning\n";

		default:
			return "Invalid FRHAL error passed, not in error list\n";
		}
	} else if ((errSrc == FR_ERRSRC_TALARMCMD) ||
		   (errSrc == FR_ERRSRC_TALAPIARM)) {
		return talGetArmErrorMessage(errSrc, errCode);
	} else if (errSrc == FR_ERRSRC_INITCALS) {
		return talGetCalErrorMessage(errSrc, errCode);
	} else if (errSrc == FR_ERRSRC_API) {
		switch (errCode) {
		case FR_ERR_OK:
			return "\n";
		case FR_ERR_INV_NULL_INIT_PARAM:
			return "FR9009 init data structure pointer is null\n";
		case FR_ERR_WAITFOREVENT_INV_PARM:
			return "waitForEvent had invalid parameter.\n";
		case FR_ERR_INIT_NULLPARAM:
			return "TALISE_initialize() was passed a NULL pointer for the device or init function parameter\n";
		case FR_ERR_INIT_INV_DEVCLK:
			return "Invalid device clock in TALISE_initialize()\n";
		case FR_ERR_CLKPLL_INV_HSDIV:
			return "Invalid CLKPLL HSDIV. (Valid 2, 2.5, 3, 4 or 5)\n";
		case FR_ERR_SETCLKPLL_INV_VCOINDEX:
			return "CLKPLL VCO frequency out of range.\n";
		case FR_ERR_SETCLKPLL_INV_NDIV:
			return "CLKPLL N divider too small, try using different device clock frequency\n";
		case FR_ERR_SETRFPLL_INV_PLLNAME:
			return "Invalid pllName requested in setRfPllFreuquency().\n";
		case FR_ERR_INV_SCALEDDEVCLK_PARAM:
			return "Could not scale device clock into range 40MHz to 80Mhz.\n";
		case FR_ERR_SETRFPLL_INV_REFCLK:
			return "Unsupported PLL reference clock or refclk out of range.\n";
		case FR_ERR_SETORXGAIN_INV_ORXPROFILE:
			return "Can not set ORx gain Index, ORX profile is not valid in device data structure\n";
		case FR_ERR_SETORXGAIN_INV_CHANNEL:
			return "Invalid ObsRx channel in setObsRxManualGain().\n";
		case FR_ERR_SETORXGAIN_INV_ORX1GAIN:
			return "Invalid ORx1 gain index requested in setObsRxManualGain()\n";
		case FR_ERR_SETORXGAIN_INV_ORX2GAIN:
			return "Invalid ORx2 gain index requested in setObsRxManualGain()\n";
		case FR_ERR_SETTXATTEN_INV_STEPSIZE_PARM:
			return "Invalid TxAtten stepsize. Use enum fr9009TxAttenStepSize_t\n";
		case FR_ERR_SETTXATTEN_INV_TXCHANNEL:
			return "Invalid Tx channel selected.\n";
		case FR_ERR_SETTXATTEN_INV_PARM:
			return "Tx attenuation is out of range (0 - 41950 mdB).\n";
		case FR_ERR_PROGRAMFIR_INV_NUMTAPS_PARM:
		...
		case FR_ERR_RXGAINTABLE_INV_CHANNEL:
			return "TALISE_programRxGainTable(): Invalid channel specified\n";
		case FR_ERR_RXGAINTABLE_INV_GAIN_INDEX_RANGE:
		...
		case FR_ERR_ORXGAINTABLE_INV_PROFILE:
		...
		case FR_ERR_ORXGAINTABLE_INV_CHANNEL:
			return "TALISE_programOrxGainTable(): Invalid channel specified\n";
		case FR_ERR_ORXGAINTABLE_INV_GAIN_INDEX_RANGE:
		...
		case FR_ERR_INV_AGC_PWR_STRUCT_INIT:
		...
		case FR_ERR_INV_AGC_PKK_HB2GAINSTEP1RECOVERY_PARAM:
		...
		case FR_ERR_INV_AGC_PKK_HB2GAINSTEPATTACK_PARAM:
		...
		case FR_ERR_INV_AGC_PWR_LWR1THRSH_PARAM:
		...
		case FR_ERR_INV_AGC_PWR_LWR0PWRGAINSTEP_PARAM:
		...
		case FR_ERR_INV_AGC_PWR_LWR1PWRGAINSTEP_PARAM:
		...
		case FR_ERR_INV_AGC_PWR_MSR_DURATION_PARAM:
		...
		case FR_ERR_INV_AGC_PWR_UPPWR0GAINSTEP_PARAM:
		...
		case FR_ERR_INV_AGC_PWR_UPPWR1GAINSTEP_PARAM:
		...
		case FR_ERR_INV_AGC_RX_PEAK_WAIT_TIME_PARM:
			return "device->rx->rxAgcCtrl->agcPeakWaitTime out of range in TALISE_setupRxAgc()\n";
		case FR_ERR_INV_AGC_RX_MIN_MAX_GAIN_PARM:
		...
		case FR_ERR_INV_AGC_RX_GAIN_UNDERRANGE_MID_INTERVAL_PARM:
		...
		case FR_ERR_INV_AGC_RX_GAIN_UNDERRANGE_HIGH_INTERVAL_PARM:
		...
		case FR_ERR_INV_RX_DEC_POWER_PARAM:
			return "TALISE_getRxDecPower: Null rxDecPower_mdBFS function parameter\n";
		case FR_ERR_GETRXDECPOWER_INV_PROFILE:
	...
	} else if (errSrc == FR_ERRSRC_FR_API_GPIO) {
...
const char* TALISE_getRegisteredErrorMessage(fr9009Device_t *device,
		uint32_t errSrc, uint32_t errCode)
{
#ifndef TALISE_VERBOSE
	return "";
	...
	if ((errSrc == FR_ERRSRC_API) ||
	    (errSrc == FR_ERRSRC_FRHAL) ||
	    (errSrc == FR_ERRSRC_TALARMCMD) ||
	    (errSrc == FR_ERRSRC_FR_API_GPIO) ||
	    (errSrc == FR_ERRSRC_TALAPIARM) ||
	    (errSrc == FR_ERRSRC_INITCALS)) {
...
talRecoveryActions_t talApiErrHandler(fr9009Device_t *device, fr9009ErrHdls_t errHdl,
		uint32_t detErr, talRecoveryActions_t retVal, talRecoveryActions_t recAction)
{
	switch (errHdl)
	{
		case FR_ERRHDL_HAL_GPIO: /* Fall through */
		case FR_ERRHDL_HAL_WAIT:
			if (detErr != FRHAL_OK)
			{
				brWriteToLog(device->devHalInfo, FRHAL_LOG_ERR, detErr,
					   TALISE_getErrorMessage(FR_ERRSRC_FRHAL, detErr));
				device->devStateInfo.errSource = FR_ERRSRC_FRHAL;
				device->devStateInfo.errCode = detErr;
				return recAction;
			}
			return retVal;

		case FR_ERRHDL_HAL_LOG:
			if (detErr != FRHAL_OK)
			{
				device->devStateInfo.errSource = FR_ERRSRC_FRHAL;
				device->devStateInfo.errCode = detErr;
		...
		case FR_ERRHDL_INVALID_PARAM: /* Fall through */
		case FR_ERRHDL_API_FAIL:
			/*Ignore Error from log as lower priority than error being handled*/
			brWriteToLog(device->devHalInfo, FRHAL_LOG_ERR, detErr,
				   TALISE_getErrorMessage(FR_ERRSRC_API, detErr));
			device->devStateInfo.errSource = FR_ERRSRC_API;
			device->devStateInfo.errCode = detErr;
			return recAction;

		case FR_ERRHDL_APIARM_ERR:
			/*Ignore Error from log as lower priority than error being handled*/
			brWriteToLog(device->devHalInfo, FRHAL_LOG_ERR, detErr,
					TALISE_getErrorMessage(FR_ERRSRC_TALAPIARM, detErr));
			device->devStateInfo.errSource = FR_ERRSRC_TALAPIARM;
			device->devStateInfo.errCode = detErr;
			return recAction;

		case FR_ERRHDL_ARM_CMD_ERR:
			return talArmCmdErrorHandler(device, errHdl, detErr, retVal, recAction);

		case FR_ERRHDL_ARM_INITCALS_ERR:
			/*Ignore Error from log as lower priority than error being handled*/
			brWriteToLog(device->devHalInfo, FRHAL_LOG_ERR, detErr,
					TALISE_getErrorMessage(FR_ERRSRC_INITCALS, detErr));
			device->devStateInfo.errSource = FR_ERRSRC_INITCALS;
			device->devStateInfo.errCode = detErr;
			return recAction;

		case FR_ERRHDL_API_GPIO:
			/*Ignore Error from log as lower priority than error being handled*/
			brWriteToLog(device->devHalInfo, FRHAL_LOG_ERR, detErr,
					TALISE_getErrorMessage(FR_ERRSRC_FR_API_GPIO, detErr));
			device->devStateInfo.errSource = FR_ERRSRC_FR_API_GPIO;
			device->devStateInfo.errCode = detErr;
			return recAction;

		case FR_ERRHDL_API_C0_PCA:
			brWriteToLog(device->devHalInfo, FRHAL_LOG_ERR, detErr,
					TALISE_getRegisteredErrorMessage(device, FR_ERRSRC_FR_API_C0_PCA, detErr));
			device->devStateInfo.errSource = FR_ERRSRC_FR_API_C0_PCA;
			device->devStateInfo.errCode = detErr;
			return recAction;

		case FR_ERRHDL_API_C0_PHMFOVR:
			 brWriteToLog(device->devHalInfo, FRHAL_LOG_ERR, detErr,
					 TALISE_getRegisteredErrorMessage(device, FR_ERRSRC_FR_API_C0_PHMFOVR, detErr));
			 device->devStateInfo.errSource = FR_ERRSRC_FR_API_C0_PHMFOVR;
			 device->devStateInfo.errCode = detErr;
			 return recAction;
