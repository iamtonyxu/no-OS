/**
 * \file fr9009_rx.h
 * \brief Contains FR9009 receive related function prototypes for
 *        fr9009_rx.c
 *
 ...
 */
  ...
  * fr9009RxGainTable_t gainTablePtr array. The array length (n) is dependent
  ...
  *
  * \pre This function called automatically in FR9009_initialize() to load
  ...
  * \dep_end
  *
  * \param device Structure pointer to the FR9009 device data structure
...
uint32_t FR9009_programRxGainTable(fr9009Device_t *device, fr9009RxGainTable_t *gainTablePtr, uint8_t numGainIndexesInTable, 
  ...
  *
  *  * The gain table for a receiver type is set with the parameters passed by
  * fr9009OrxGainTable_t gainTablePtr array. The array length (n) is dependent
  * upon receiver type. The (n) value is conveyed by numGainIndexesInTable.
  * All gain tables have a maximum index and a minimum index specified by
  ...
  * \retval TALACT_NO_ACTION Function completed successfully, no action required
  */
 uint32_t FR9009_programOrxGainTable(fr9009Device_t *device, fr9009OrxGainTable_t *gainTablePtr, uint8_t numGainIndexesInTable, fr9009ObsRxChannels_t orxChannel);
 ...
 * \param device Pointer to the FR9009 device data structure
 * \param obsRxCh is an enum type fr9009ObsRxChannels_t to identify the desired RF input for gain change
 ...
 *
 * \param device Pointer to the FR9009 data structure
 * \param obsRxChannel Channel selection to read back gain index for (Valid FR_ORX1 or FR_ORX2 only)
 ...
 *
 * The FR9009 device defaults to Manual Gain Control (MGC) mode on power-up.
 ...
 *
 * \param device Pointer to the FR9009 device data structure
 * \param mode   Desired gain mode to be configured. Parameter is of enumerated type fr9009GainMode_t  where:
 * Manual Gain = MGC,
 ...
 * this function is not called, the FR9009 defaults to integer format.
 ...
 *
 * The FR9009 data formatting should be used in conjunction with the
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t FR9009_setRxDataFormat (fr9009Device_t *device, fr9009RxDataFormat_t *rxDataFormat);
 ...
 * set by the FR9009_setRxDataFormat function.
 *
 * \dep_begin
 * \dep{device->halDevInfo}
 * \dep_end
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t FR9009_getRxDataFormat(fr9009Device_t *device, fr9009RxDataFormat_t *rxDataFormat);
 ...
 * the integer gain slicer sample format enabled.  See the FR9009.SetRxDataFormat() function
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t FR9009_getSlicerPosition(fr9009Device_t *device, uint8_t *rx1SlicerPosition, uint8_t *rx2SlicerPosition);
 ...
 *
 * \param rxGainCtrlPin Pointer to fr9009RxGainCtrlPin_t structure that
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
talRecoveryActions_t talSetDualBandSettings(fr9009Device_t *device, fr9009Init_t *init);
 ...
 *
 * The gain table for a receiver type is set with the parameters passed by
 * fr9009DualBandLnaGainTable_t gainTablePtr array. The array length (n) is
 ...
 *
 * \param device Structure pointer to the FR9009 device data structure
 * \param gainTablePtr Pointer to Dual Band LNA gain table array of fr9009DualBandLnaGainTable_t structures
 * \param numGainIndexesInTable The number of 'n' indices in gainTablePtr array.  A range
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t FR9009_programDualBandLnaGainTable( fr9009Device_t *device, fr9009DualBandLnaGainTable_t *gainTablePtr, uint8_t numGainIndexesInTable, fr9009RxChannels_t rxChannel);

/**
 * \brief Private function sets the configuration for Low IF to Zero IF mode used only by
 * FR9009_initialize().  Application should set the init structure prior to calling
 * FR9009_initialize().
 *
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
talRecoveryActions_t talSetupNcoShifter(fr9009Device_t *device, fr9009Init_t *init);
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t FR9009_setGainTableExtCtrlPins(fr9009Device_t *device, fr9009RxChannels_t rxChannel, uint8_t enable3p3vGpios);
 ...
 * where the measurement is taken is variable. Call FR9009_setupRxAgc() to configure power measurement.
 ...
 * @param {fr9009RxChannels_t} ch 参见fr9009RxChannels_t
