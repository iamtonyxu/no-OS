/**
 * \file fr9009_arm.h
 * \brief Contains FR9009 ARM related function prototypes for fr9009_arm.c
 *
...
#ifndef FR9009_ARM_H_
#define FR9009_ARM_H_
 ...
 * \dep_end
 *
 * \param device Pointer to the FR9009 device settings data structure
 * \param init Pointer to the FR9009 initialization setting data structure
 *
...
/**
 * \brief Writes the FR9009 ARM configuration settings
 *
 * \pre This function is called automatically during FR9009_initArm(), and
 ...
 *
 * \param device Pointer to the FR9009 device settings data structure
 * \param init Pointer to the FR9009 initialization settings data structure
 *
 * \retval TALACT_WARN_RESET_LOG Recovery action for log reset
 * \retval TALACT_ERR_CHECK_PARAM Recovery action for bad parameter check
 * \retval TALACT_ERR_RESET_SPI Recovery action for SPI reset required
 ...
 */
 ...
 /**
  * \brief Read from the FR9009 ARM program or data memory
  *
...
uint32_t FR9009_readArmMem(fr9009Device_t *device, uint32_t address, uint32_t *returnDataWord, uint32_t WordsToRead, uint8_t autoIncrement);
 ...
 /**
  * \brief Write to the FR9009 ARM program or data memory
  *
...
uint32_t FR9009_writeArmMem(fr9009Device_t *device, uint32_t address, uint32_t *dataWord, uint32_t WordCount);
 ...
 /**
  * \brief Low level helper function used by FR9009 API to write the ARM memory config structures
  *
  * Normally this function should not be required to be used directly by the BBIC.  This is a helper
  * function used by other FR9009 API commands to write settings into the ARM memory.
  *
  * \pre This function is private and is not called directly by the user.
  *
  * \dep_begin
 ...
 */
 ...
 /**
  * \brief Low level helper function used by FR9009 API to read the ARM memory config structures
  *
  * Normally this function should not be required to be used directly by the BBIC.  This is a helper
  * function used by other FR9009 API commands to read settings from the ARM memory.
  *
  * \pre This function is private and is not called directly by the user
  *
  * \dep_begin
 ...
 /**
  * \brief Reads the FR9009 ARM  command status register and returns an error and status word
  *
 ...
 /**
  * \brief Isolated byte read of the FR9009 ARM  command status register based on the opcode
  *
  ...
  * \retval TALACT_NO_ACTION Function completed successfully, no action required
  */
// uint32_t FR9009_readArmCmdStatusByte(fr9009Device_t *device, uint8_t opCode, uint8_t *cmdStatByte);
uint32_t FR9009_readArmCmdStatusWord(fr9009Device_t *device, uint32_t opCode, uint32_t *cmdStatword);

 /**
  * \brief FR9009 ARM command status wait function polls command status register until opcode of interest completes
  *
  ...
  * \retval TALACT_NO_ACTION Function completed successfully, no action required
  */
// uint32_t FR9009_waitArmCmdStatus(fr9009Device_t *device, uint8_t opCode, uint8_t *cmdStatByte, uint32_t timeout_us, uint32_t waitInterval_us);
...
uint32_t FR9009_ReadWait(fr9009Device_t *device, uint32_t readaddr, uint32_t waitval, uint32_t *rd_word, uint32_t timeout_us, uint32_t waitInterval_us);
 ...
 /**
  * \brief Sends a command to the FR9009 ARM processor interface
  *
  ...
  * \retval TALACT_NO_ACTION Function completed successfully, no action required
  */
// uint32_t FR9009_sendArmCommand(fr9009Device_t *device, uint8_t opCode, const uint8_t *extendedData, uint8_t extendedDataNumBytes);
 ...
 */
 ...
 /**
 * \brief Reads back the version of the ARM binary loaded into the FR9009 ARM memory
 *
 ...
 * Note: only 'FR_ARM_BUILD_RELEASE' build type should be used with production code
 *
 * <B>Dependencies</B>
...
/**
* \deprecated Please see FR9009_getArmVersion_v2 for current implementation
*
* \brief ***DEPRECATED*** Reads back the version of the ARM binary loaded into the FR9009 ARM memory
*
* This function reads the ARM memory to read back the major.minor.releaseCandidate
...
/*
* \brief FR9009_ArmWriteField used after 3109 is start
* 
...
uint32_t FR9009_ArmWriteField(fr9009Device_t *device, uint32_t addr, uint32_t fieldVal,
 ...
 * checksum in the FR9009 ARM after the binary file has been loaded.  This function
 * will wait for a timeout period for the checksum calculation to occur.  The
 * user can adjust the timeout period and SPI read interval in fr9009_user.c by
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
//uint32_t FR9009_verifyArmChecksum(fr9009Device_t *device);
