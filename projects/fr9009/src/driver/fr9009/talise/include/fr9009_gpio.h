...
#ifndef FR9009_GPIO_H_
#define FR9009_GPIO_H_
...
#include "fr9009_gpio_types.h"

/**
 * \brief Sets the FR9009 low voltage GPIO pin directions
 *
...
/**
 * \brief Gets the FR9009 low voltage GPIO pin directions
 *
 ...
 * function parameter returns a bit per GPIO pin.  1 = output from the FR9009
 * Device, 0 = input into the FR9009 device.
 *
 *
 * \dep_begin
 * \dep{device->devHalInfo}
...
/**
 * \brief Sets the FR9009 GPIO output source for different GPIO functionality
 *
 ...
 * fr9009GpioMode_t can be bitshifted and bitwise OR-ed together to create the
...
/**
 * \brief Reads the FR9009 GPIO output source for each set of four low voltage
 *        GPIO pins.
 *
 *  See ENUM fr9009GpioMode_t for possible GPIO output sources.
 *
 * \dep_begin
 * \dep{device->devHalInfo}
 * \dep_end
...
/**
 * \brief Sets the FR9009 low voltage GPIO output pins level
 *
...
/**
 * \brief Reads the FR9009 low voltage GPIO pin levels and returns their
...
/**
 * \brief Reads the FR9009 GPIO pin output levels for BITBANG mode
 *
 ...
 *
 * The monitor outputs allow visibility to some internal FR9009 signals.  Each
...
/**
 * \brief This API function reads the GPIO monitor index and mask from FR9009
 *
 * \dep_begin
 * \dep{device->devHalInfo}
 * \dep_end
 ...
 * FR9009_setGpIntMask(device, ~FR_GP_MASK_JESD_DEFRMER_IRQ);
 *
 ...
 * fr9009GpIntMask_t enumerated types are or'd together to form the GP interrupt mask word.
 ...
 *                 [14] | Reserved
 *                 [13] | Reserved
 *                 [12] | FR_GP_MASK_STREAM_ERROR
 *                 [11] | FR_GP_MASK_ARM_CALIBRATION_ERROR
 *                 [10] | FR_GP_MASK_ARM_SYSTEM_ERROR
 *                  [9] | FR_GP_MASK_ARM_FORCE_INTERRUPT
 *                  [8] | FR_GP_MASK_WATCHDOG_TIMEOUT
 *                  [7] | FR_GP_MASK_PA_PROTECTION_TX2_ERROR
 *                  [6] | FR_GP_MASK_PA_PROTECTION_TX1_ERROR
 *                  [5] | FR_GP_MASK_JESD_DEFRMER_IRQ
 *                  [4] | FR_GP_MASK_JESD_FRAMER_IRQ
 *                  [3] | FR_GP_MASK_CLK_SYNTH_NONLOCK_ERROR
 *                  [2] | FR_GP_MASK_AUX_SYNTH_NONLOCK_ERROR
 *                  [1] | FR_GP_MASK_RF_SYNTH_NONLOCK_ERROR
 *                  [0] | Reserved
 * 
 * \note The AUXPLL Interrupt is masked in FR9009_Initialize() since the ARM routinely
 ...
 *                 [14] | Reserved
 *                 [13] | Reserved
 *                 [12] | FR_GP_MASK_STREAM_ERROR (1 = IRQ output masked (disabled); 0 = IRQ output unmasked (enabled))
 *                 [11] | FR_GP_MASK_ARM_CALIBRATION_ERROR (1 = IRQ output masked (disabled); 0 = IRQ output unmasked (enabled))
 *                 [10] | FR_GP_MASK_ARM_SYSTEM_ERROR (1 = IRQ output masked (disabled); 0 = IRQ output unmasked (enabled))
 *                  [9] | FR_GP_MASK_ARM_FORCE_INTERRUPT (1 = IRQ output masked (disabled); 0 = IRQ output unmasked (enabled))
 *                  [8] | FR_GP_MASK_WATCHDOG_TIMEOUT (1 = IRQ output masked (disabled); 0 = IRQ output unmasked (enabled))
 *                  [7] | FR_GP_MASK_PA_PROTECTION_TX2_ERROR (1 = IRQ output masked (disabled); 0 = IRQ output unmasked (enabled))
 *                  [6] | FR_GP_MASK_PA_PROTECTION_TX1_ERROR (1 = IRQ output masked (disabled); 0 = IRQ output unmasked (enabled))
 *                  [5] | FR_GP_MASK_JESD_DEFRMER_IRQ (1 = IRQ output masked (disabled); 0 = IRQ output unmasked (enabled))
 *                  [4] | FR_GP_MASK_JESD_FRAMER_IRQ (1 = IRQ output masked (disabled); 0 = IRQ output unmasked (enabled))
 *                  [3] | FR_GP_MASK_CLK_SYNTH_NONLOCK_ERROR (1 = IRQ output masked (disabled); 0 = IRQ output unmasked (enabled))
 *                  [2] | FR_GP_MASK_AUX_SYNTH_NONLOCK_ERROR (1 = IRQ output masked (disabled); 0 = IRQ output unmasked (enabled))
 *                  [1] | FR_GP_MASK_RF_SYNTH_NONLOCK_ERROR (1 = IRQ output masked (disabled); 0 = IRQ output unmasked (enabled))
 *                  [0] | Reserved
 * 
 * \retval TALACT_WARN_RESET_LOG Recovery action for log reset
 * \retval TALACT_ERR_CHECK_PARAM Recovery action for bad parameter check
 ...
 *                  [5] | FR_GP_MASK_JESD_DEFRMER_IRQ
 *                  [4] | FR_GP_MASK_JESD_FRAMER_IRQ
 *                  [3] | FR_GP_MASK_CLK_SYNTH_NONLOCK_ERROR (1 = unlocked)
 *                  [2] | FR_GP_MASK_AUX_SYNTH_NONLOCK_ERROR (1 = unlocked)
 *                  [1] | FR_GP_MASK_RF_SYNTH_NONLOCK_ERROR (1 = unlocked)
 *                  [0] | Reserved
 *
 * \retval TALACT_WARN_RESET_LOG Recovery action for log reset
 * \retval TALACT_ERR_CHECK_PARAM Recovery action for bad parameter check
...
/**
 * \brief Reads back the device temparature sensor
 *
 ...
 *
 * \param device Pointer to the FR9009 data structure
 * \param temparatureDegC A pointer to a single uint16_t element that returns the
 *        current 12bit temparature sensor value in degrees C.
 *
 * \retval TALACT_WARN_RESET_LOG recovery action for log reset
 * \retval TALACT_ERR_CHECK_PARAM recovery action for bad parameter check
 * \retval TALACT_ERR_RESET_SPI recovery action for SPI reset required
...
/**
 * \brief Sets up the 10 AuxDACs on the FR9009 device.
 *
 * This function uses the configuration in the FR9009 auxDac data structure
 * function parameter to setup each of the twelve DACs
 *
 * This function can be called any time after FR9009_initialize() to reconfigure, enable, disable the
 * different DAC outputs. The DACs are used in manual control mode.  After calling this setup
 * function, it is possible to change a specific DAC code by calling the FR9009_writeAuxDac() function.
 *
 ...
 *
 * \param device Pointer to the FR9009 data structure
 * \param auxDac Pointer to the FR9009 auxiliary DAC data structure
 *
 * \retval TALACT_WARN_RESET_LOG Recovery action for log reset
 * \retval TALACT_ERR_CHECK_PARAM Recovery action for bad parameter check
 * \retval TALACT_ERR_RESET_SPI Recovery action for SPI reset required
...
/**
 * \brief Enables/Disables SPI2 on FR9009
 *
 * The FR9009 device can enable a second SPI port on the low voltage GPIO[3:0]
 ...
 *
 * \param device Pointer to the FR9009 data structure
 * \param spi2Enable Enable(=1)/Disable(=0) SPI2 protocol on FR9009
...
/**
 * \brief Get current status of SPI2 configuration on FR9009
 *
 ...
 *
 * \param device Pointer to the FR9009 data structure
 * \param spi2Enable Indicates whether SPI2 protocol is Enabled(=1)/Disabled(=0) on FR9009
 * SPI2 仅对GPIO[3:0]这个组有效，
 *	GPIO_0: SPI2_DIO/MOSI (支持双向模式)
 *	GPIO_1: SPI2_DOUT
 *	GPIO_2: SPI2_CLK
...
/**
 * \brief Sets the FR9009 3.3v GPIO pin directions
 *
...
/**
 * \brief Sets the FR9009 3.3V GPIO output source for different GPIO functionality
 *
 * This function will only affect the GPIO pins that have their OE direction
 * set to output. Each group of four GPIO pins can be assigned a GPIO source.  Each
 * GPIO nibble (4 pins) must share that same GPIO output source. ENUM
 * fr9009Gpio3v3Mode_t can be bitshifted and bitwise OR-ed together to create the
 * value for the gpioSrcCtrl function parameter.
 *
 * \dep_begin
 * \dep{device->devHalInfo}
...
/**
 * \brief Reads the FR9009 3.3V GPIO output source for different GPIO functionality
 *
 *  See ENUM fr9009Gpio3v3Mode_t for possible GPIO output sources.
 *
 * \dep_begin
 * \dep{device->devHalInfo}
 * \dep_end
...
/**
 * \brief Sets the FR9009 3.3V GPIO output pins level
 *
 * This function will only affect the GPIO pins that have their OE direction set to output and
 * that have the correct source control for the nibbles in FR_GPIO3V3_BITBANG_MODE
 *
 * \dep_begin
 * \dep{device->devHalInfo}
 * \dep_end
...
/**
 * \brief Reads the FR9009 3.3V GPIO pin levels
 *
...
uint32_t FR9009_gpIntHandler(fr9009Device_t *device, uint32_t *gpIntStatus, fr9009GpIntInformation_t *gpIntDiag);
  ...
  *
  *  NOTE: This API is only supported when the ARM processor in FR9009 is in IDLE state.
  *  Hence, it is required to call FR9009_RadioOff() before setting up the
 ...
 *
 *  Only GPIO[0] to GPIO[15] are valid GPIO pin selects. FR_GPIO_INVALID
 ...
 *
 * \param device Pointer to the FR9009 data structure
 * \param pinModeGpio pointer to fr9009GpioPinSel_t enum type to which GPIO pin currently
...
uint32_t FR9009_getAuxAdcPinModeGpio(fr9009Device_t *device, fr9009GpioPinSel_t *pinModeGpio);
 ...
 /**
  * \brief Configures one of the four 12-bit AuxADC on the FR9009 device for sampling and A/D conversion.
  * AuxADC on FR9009 is also used by the temparature sensor. This API configures FR9009 to allow external
  * use of Aux ADC.
  *
  * This function uses the configuration in the fr9009AuxAdcConfig_t data structure
  * function parameter to setup the AuxADC channel to be used for external A/D conversion use.
  *
  * This function can be called any time after FR9009_initialize() to schedule sampling and conversion
  * of AuxADC inputs. AuxADC A/D conversion results can be obtained through FR9009_readAuxAdc() API call.
  *
  * To setup the GPIO pin for Pin mode Aux ADC conversion, use FR9009_setAuxAdcPinModeGpio() API.
  * The ARM processor on FR9009 needs to be in IDLE state to accept the GPIO ctrl command.
  * Hence, it is reqd to call the FR9009_RadioOff() API before calling FR9009_setAuxAdcPinModeGpio() API
  * following which FR9009_RadioOn() API can be called to move the ARM processor in FR9009 to RADIO_ON state.
  *
  * The Pin-mode AuxADC conversion expects a low(0) to high(1) pulse on the GPIO pin assigned to Aux ADC
  * start signal via FR9009_setAuxAdcPinModeGpio() API to schedule A/D Conversion.A rising edge on this pin is needed for
  ...
  *
  * NOTE: FR9009_startAuxAdc() will work in Radio ON or Radio OFF state
  *
  * \dep_begin
  * \dep{device->devHalInfo}
  * \dep_end
  *
  * \param device Pointer to the FR9009 data structure
  * \param auxAdcConfig Pointer to fr9009AuxAdcConfig_t FR9009 auxiliary ADC configuration data structure
  *
  * \retval TALACT_WARN_RESET_LOG Recovery action for log reset
  * \retval TALACT_ERR_CHECK_PARAM Recovery action for bad parameter check
  * \retval TALACT_ERR_RESET_SPI Recovery action for SPI reset required
  * \retval TALACT_NO_ACTION Function completed successfully, no action required
  */
uint32_t FR9009_startAuxAdc(fr9009Device_t *device, fr9009AuxAdcConfig_t *auxAdcConfig);
 ...
 * data structure.
 *
 * This function can be called any time after FR9009_initialize() and following a FR9009_startAuxAdc() call.
 *
 ...
 *
 * NOTE: FR9009_readAuxAdc() will work in Radio ON or Radio OFF state
 *
 * \dep_begin
 * \dep{device->devHalInfo}
 * \dep_end
 *
 * \param device Pointer to the FR9009 data structure
 * \param auxAdcResult Pointer to the fr9009AuxAdcResult_t data structure where the result
 ...
 *  error strings to the FR9009_getErrorMessage function.
 *
 *  This is a private function and is automatically called by the API.  BBIC
 *  should use the FR9009_getErrorMessage function.
 *
 * \param errSrc A value that represents the error source from the FR9009 API
...
/*!
 * \brief Helper function to set the register of FR9009ES2 ORX_TX_PIN_SEL
 ...
 * 
 * \param device device Pointer to the FR9009 data structure
 * \param config 4 * fr9009GpioShortPinSel_t
 * \retval TALACT_WARN_RESET_LOG Recovery action for log reset
 * \retval TALACT_ERR_CHECK_PARAM Recovery action for bad parameter check
 * \retval TALACT_ERR_RESET_SPI Recovery action for SPI reset required
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
