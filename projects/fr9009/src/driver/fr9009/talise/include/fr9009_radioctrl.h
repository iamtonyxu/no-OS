/**
 * \file fr9009_radioctrl.h
 * \brief Contains FR9009 related function prototypes for fr9009_radioctrl.c
 *
...
#ifndef FR9009_RADIOCTRL_H_
#define FR9009_RADIOCTRL_H_
 ...
 */
  ...
  * \dep_end
  *
  * \param device is structure pointer to the FR9009 data structure containing settings
...
uint32_t FR9009_loadStreamFromBinary(fr9009Device_t *device, uint8_t *binary);
 ...
 /**
  * \brief Instructs the FR9009 ARM which GPIO pins are used for TDD pin control
  *
  * The FR9009 can control any of these related signals by sending an ARM command, or
  * the FR9009 ARM signals can be routed to GPIO pins.  Each signal in the armGpio
  * structure has an enable member.  If the enable member is set, the FR9009 ARM
  ...
  * FR9009_loadArmFromBinary() function call.  If the BBP wishes to change the GPIO
  * assignments this function can be called again to change the configuration while
  * the ARM is in the radioOff state.  This function also sets the FR9009 GPIO pin
  * direction for any GPIO pins that are enabled in this function.
  *
  * \pre This function can be called after loading FR9009 ARM binary, but before
 ...
 * \param orxChannel fr9009ObsRxChannels_t enum type to select either ORx1, ORx2,
 * ORx1 + ORx2 gain table for programming.  A channel check is performed to
 * ensure a valid selection.
 *  
 * \param orxEnGpioPinSel Pointer to fr9009GpioShortPinSel_t structure that
 ...
 *
 *  If this feature is enabled, note that the FR9009 trackings calibrations
 *  will be limited or not usable, as some of the FR9009 tracking cals require
 *  using the AuxLO.
 *
 *  If this function is enabled to set the ORx LO source to AuxLO, the BBIC can
 *  control the ORx Lo source either using the FR9009_setOrxLoSource() function
 ...
 *  is recommended. Assigning the orxLoCfg->gpioSelect = FR_GPIO_INVALID
 *  disables pin mode.
 *
 ...
 * \retval TALACT_NO_ACTION function completed successfully, no action required
 */
uint32_t FR9009_setOrxLoCfg(fr9009Device_t *device, const fr9009OrxLoCfg_t *orxLoCfg);
 ...
 *  FR9009_setOrxLoCfg() function.
 *
 * \pre This function can be called anytime after FR9009_setOrxLoCfg() has been
 ...
 * off-line state, call the FR9009_radioOff() function.
 *
 ...
 * keep the receive and transmit chains powered down until the FR9009_radioOn() function
 * is called again.
 *
 * \pre This function may be called any time FR9009_radioOn had been called previously
 *
 * \dep_begin
 * \dep{device->devHalInfo}
 * \dep_end
...
uint32_t FR9009_setRxTxEnable(fr9009Device_t *device, fr9009RxORxChannels_t rxOrxChannel, fr9009TxChannels_t txChannel);
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t FR9009_getRxTxEnable(fr9009Device_t *device, fr9009RxORxChannels_t *rxOrxChannel, fr9009TxChannels_t *txChannel);
/**
 ...
 *
 * \param oRx1Map enumerated type fr9009TxToOrxMapping_t which maps either Tx1 or Tx2 to ORx1.
 * \param oRx2Map enumerated type fr9009TxToOrxMapping_t which maps either Tx1 or Tx2 to ORx2.
 *
 * Both oRx1Map and oRx2Map use the fr9009TxToOrxMapping_t enumerated type encoded as a bit-field:
 * FR_MAP_NONE    = 0x00 = No Tx at ORx input
 * (No ENUM)       = 0x01 = No Tx at ORx input
 * FR_MAP_TX1_ORX = 0x02 = Tx1 mapped to enabled ORx
 * FR_MAP_TX2_ORX = 0x03 = Tx2 mapped to enabled ORx
 *
 * \retval TALACT_ERR_CHECK_PARAM Recovery action for bad parameter check
 * \retval TALACT_ERR_CHECK_TIMER Recovery action for timer time-out check required
 * \retval TALACT_ERR_RESET_SPI Recovery action for SPI reset required
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t FR9009_setTxToOrxMapping(fr9009Device_t *device, uint8_t txCalEnable, fr9009TxToOrxMapping_t oRx1Map, fr9009TxToOrxMapping_t oRx2Map);
 ...
 * \param pllName Name of the PLL to configure based on the enumerated types in fr9009RfPllName_t
 ...
 *
 * This function is used to get a PLL's current frequency. A fr9009RfPllName_t enumerated type is passed for
 ...
 *
 * This function returns the status of the FR9009 PLLs via the pllLockStatus pointer.
 ...
 *
 * \deprecated This function is deprecated.  FR9009_setPllLoopFilter should be
 ...
 * FR9009_setRfPllFrequency function for the FR_RF_PLL to set up the RF PLL
 ...
 *
 * \deprecated This function is deprecated.  FR9009_getPllLoopFilter should be
 ...
 * \pre This function can be called after the ARM has been initialized, and
 * device must be in radioOff state. This function must be followed with a
 * FR9009_setRfPllFrequency function for the selected PLL to set up the PLL
 ...
 *
 * Please note that only FR_GPIO_00 - FR_GPIO_15 are valid for frequency
 * hop trigger GPIO pin. If FR_GPIO_INVALID is set, then the GPIO is
...
/**
 * \brief This function configures frequency start or end with cammand mode
 * \param device Pointer to the FR9009 device data structure containing settings
 * \param start_en 1: start , 0: end fast hop 
*/
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t FR9009_getFhmConfig(fr9009Device_t *device, fr9009FhmMode_t *fhmConfig);
 ...
 *
 * \pre This function can be used after the device has been fully initialized
 *      and the ARM processor is initialized and FR9009_setFhmConfig() is called.
 ...
 *      FR9009_setFhmConfig() API and the FHM mode is set via
 *      FR9009_setFhmMode() API. This API can be called only in Radio ON state
 *
 * \dep_begin
 * \dep{device->devHalInfo}
 * \dep_end
 ...
 *
 * This function can be used to read the current FHM status. Please refer to fr9009FhmStatus_t structure definition
...
/**
 * \brief This function enables/disables the external LO input and sets the
 *        external LO input divider.If the internal LO used RFPLL 
 * 
 * If the internal LO dividers for the RFPLL are changed with
 * FR9009_setRfPllFrequency() and the internal LO divider changes, it is likely this
 * function would need to be called again to scale extLoDivide as well to
 ...
 *
 * \param device Pointer to the FR9009 device data structure containing settings
 * \param enableExtLo Enable/Disable the LO input buffer.,if  ext freq > 1Ghz, set 0x01 enable, 
 *        else set 0x11 enable , 0: disable
 * \param extLoDivide Sets external LO input frequency = (ect input/extLoOutDivide)
 */
