/**
 * \file fr9009_tx.h
 * \brief Contains FR9009 transmit related function prototypes for
 *        fr9009_tx.c
 *
 ...
 */
  ...
  * \dep_end
  *
  * \param device Pointer to the FR9009 device's data structure
  * \param txChannel should be one of the enum fr9009TxAttChannels_t, note that TXOFF wont have any effect. Only can set one channel at a time.
...
uint32_t FR9009_setTxDigAttenuation(fr9009Device_t *device, fr9009TxChannels_t txChannel, uint16_t txAttenuation_mdB);
  ...
  *
  * \param device Pointer to the FR9009 device's data structure
  * \param txChannel Is one of the enum fr9009TxAttChannels_t channel types, where only FR_TX1 or FR_TX2 are valid choices.
  *  Note that FR_TX_OFF will not have any effect.
 ...
 * default setting.
 *
 * \pre This function must be called before loading the FR9009 ARM processor
 *
 * \param device Pointer to the FR9009 device's data structure
 * \param dacFullScale Is of the enum fr9009DacFullScale_t that are the options
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t FR9009_setDacFullScale(fr9009Device_t *device, fr9009DacFullScale_t dacFullScale);

/**
 * \brief Enables/Disables the Tx NCO test tone
 *
 *  This function enables/disables a digital numerically controlled oscillator
 *  in the FR9009 Digital to create a test CW tone on Tx1 and Tx2 RF outputs.
 *
 ...
 *
 * \param device Pointer to the FR9009 device's data structure
 * \param txNcoTestToneCfg Pointer to fr9009TxNcoTestToneCfg_t structure that
 ...
 *  function
 *
 * This api is similar in functionality to FR9009_enableTxNco but it has a few 
 ...
 *
 * \param device Pointer to the FR9009 device's data structure
 * \param txNcoTestToneCfg Pointer to fr9009TxNcoShifterCfg_t structure that
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t FR9009_txNcoShifterSet(fr9009Device_t *device, fr9009TxNcoShifterCfg_t *txNcoTestToneCfg);
 ...
 *
 * \param txChannel fr9009TxChannels_t enum type to select either Tx1 or Tx2
 *  channel for programming.
 *
 * \param txAttenCtrlPin Pointer to fr9009TxAttenCtrlPin_t structure that
...
/**
 * \brief Writes FR9009 device registers with settings for the PA Protection feature.
 *
 ...
 * FR9009_enablePaProtection() function.
 *
 ...
 * average duration.
 *
 * \pre Complete normal FR9009 initialization and init cals before running this function.
 *
 * \dep_begin
 * \dep{device->halDevInfo}
 * \dep_end
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t FR9009_setPaProtectionCfg(fr9009Device_t *device, fr9009TxPaProtectCfg_t *txPaProtectCfg);

/**
 * \brief Reads FR9009 device registers to retrieve settings for the PA Protection feature.
 *
 * The PA Protection feature allows for error flags to go high if the
 * average TX IQ sample power (before interpolation at JESD204 interface) in
 * the data path exceeds a programmable threshold level based on samples taken
 ...
 *
 * \pre Complete normal FR9009 initialization and init cals before running this function.
 *      Also, call FR9009_setPaProtectionCfg() to set initial configuration.
 *
 * \dep_begin
 * \dep{device->halDevInfo}
 * \dep_end
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t FR9009_getPaProtectionCfg(fr9009Device_t *device, fr9009TxPaProtectCfg_t *txPaProtectCfg);
 ...
 * are asserted.
 *
 * \pre Complete normal FR9009 initialization and init cals, and call
 *      FR9009_setPaProtectionCfg() before using FR9009_enablePaProtection()
 *
 * \dep_begin
 * \dep{device->halDevInfo}
 * \dep_end
 ...
 *        based on settings from FR9009_setPaProtectionCfg()
 *
 ...
 *
 * \pre Complete normal FR9009 initialization and init cals, and call
 *      FR9009_setPaProtectionCfg() before using FR9009_getTxSamplePower()
 *
 * \dep_begin
 * \dep{device->halDevInfo}
 * \dep_end
 *
 * \param device Pointer to the device settings structure
 * \param txChannel FR_TX1(1)=Read power from Tx1, FR_TX2(2)=Read power from Tx2
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t FR9009_getTxSamplePower(fr9009Device_t *device, fr9009TxChannels_t txChannel, uint16_t *channelPower);
 ...
 *
 * \pre Complete normal FR9009 initialization and init cals, and call
 *      FR9009_setPaProtectionCfg() first
 *
 * \dep_begin
 * \dep{device->halDevInfo}
 * \dep_end
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t FR9009_getPaProtectErrorFlags(fr9009Device_t *device, uint8_t *errorFlags);

/**
 * \brief Clears both Tx1 and Tx2 PA Protection error IRQ flags
 *
 * If Tx PA Protection is configured and enabled, the error flags per channel
 * can be read back using the FR9009_getPaProtectErrorFlags function. The
 ...
 *
 * Calling FR9009_setPaProtectionCfg() will also clear the error flags at the
 ...
 * @param {fr9009TxChannels_t} ch fr9009TxChannels_t
 * @param {uint8_t} hb_bypass 0：half band filter not bypass 1:half band filter bypass
 * @param {uint8_t} fe_bypass 0: Front-End not bypass 1:Front-End bypass
 * @param {uint8_t} be_bypass 0：Back-End not bypass 1:Back-End bypass
 * @param {fr9009IfConvCfg_t*} pIfconvCfg 参见fr9009IfConvCfg_t
 */
